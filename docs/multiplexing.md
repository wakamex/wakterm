...
On Linux, a client connecting from a different user, mount, PID, or IPC namespace receives metadata-only mux access. It can inspect pane layout, titles, agent identity, harness, lifecycle, and resource status, but terminal contents, scrollback, images, agent prompts, output, requests, events, and all control requests are rejected. Wakterm derives this authority from the kernel's Unix-socket peer credentials, pidfd, and namespaces before reading client-supplied identity fields. Restricted clients are registered under a server-generated identity, client-list responses omit SSH-agent socket paths, and agent status responses omit output-derived final messages, prompt hashes, progress summaries, and observer errors.

Metadata access still exposes pane titles, working directories, process and harness identity, lifecycle state, layout, and resource status. The namespace check detects a known Linux sandbox boundary but does not prove that a same-namespace process is unrestricted. Confinement mechanisms that do not create namespaces, inherited mux descriptors, authenticated TLS credentials, and SSH or TCP proxies require enforcement by the sandbox or credential owner. Other operating systems currently rely on their existing local socket access controls rather than Linux namespace classification.
