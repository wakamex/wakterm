...
Run the command [`wakterm.plugin.update_all()`](../../reference/lua/wakterm.plugin/update_all.md) to update all local plugins.
...
When a plugin is first referenced, [`wakterm.plugin.require()`](../../reference/lua/wakterm.plugin/require.md) will clone the repo if it doesn't already
exist and store it in the runtime directory under `plugins/NAME` where
`NAME` is derived from the repo URL.

You can discover locations of the various plugins with [`wakterm.plugin.list()`](../../reference/lua/wakterm.plugin/list.md).
...
!!! Info
    When changes are made to the local project, [`wakterm.plugin.update_all()`](../../reference/lua/wakterm.plugin/update_all.md) must be run
