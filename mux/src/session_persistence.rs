...
fn restored_harness_then_shell(command: CommandBuilder) -> anyhow::Result<CommandBuilder> {
    ...
    if shell == "/usr/bin/wsh" || shell == "/bin/wsh" {
        let mut argv = vec![shell, "--run".into(), "--login".into(), "--".into()];
...
mod test {
    ...
    #[cfg(unix)]
    #[test]
    #[ignore = "requires WAKTERM_TEST_WSH pointing to a native Wsh build and python3"]
    fn restored_harness_real_wsh_preserves_argv_and_job_control() {
        use std::os::unix::ffi::OsStringExt;
        let wsh = std::env::var_os("WAKTERM_TEST_WSH")
            .expect("set WAKTERM_TEST_WSH to the native Wsh executable");
        let probe =
            std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("test-data/wsh_restore_pty.py");
        for mode in ["exit", "interrupt"] {
            let mut command = CommandBuilder::from_argv(vec![
            ...
            command.env("SHELL", "/usr/bin/wsh");
            let wrapped = restored_harness_then_shell(command).unwrap();
            // Substitute only the executable location, preserving every
            // argument generated by the production restore caller.
            let output = std::process::Command::new("python3")
                .arg(&probe)
                .arg(&wsh)
                .args(&wrapped.get_argv()[1..])
                .output()
                .unwrap();
            assert!(
